...
public class DependencyGraph : IDependencyGraph
{
  ...
  private DependencyGraph(IEnumerable<Node> nodes)
  {
    ...
    foreach (INode node in _nodes) ((Node)node).ResolveEdges(this);

    var cycles = _nodes.FindCycles();
    foreach (INode node in _nodes) ((Node)node).IsInCycle = cycles.Exists(n => n.Contains(node));
  ...
  public INode this[string name]
  {
    get
    {
      try
      {
        return _nodes.SingleOrDefault(node => node.Name.EndsWith(name)) ?? throw new KeyNotFoundException($"No node found with name {name}");
      }
      catch
      {
  ...
  {
    return _nodes.Find(node => node.Name.Equals(name)) ?? _nodes.Find(node => node.Name.Equals('.' + name));
  }
}