...
public static class CycleFinder
{
    public static List<INode[]> FindCycles(this List<INode> nodes)
    {
        ...
        {
            DFS(node, visited, stack, cycleSet, cycles);
        }

        return cycles;
    }
    private static void DFS(INode node, HashSet<INode> visited, List<INode> stack, HashSet<string> cycleSet, List<INode[]> cycles)
    {
        ...
        {
            DFS(edge.to, visited, stack, cycleSet, cycles);
        }
    ...
    private static INode[] ExtractCycle(INode node, List<INode> stack)
    {
        ...
        int startIndex = stack.IndexOf(node);
        // Calculate the length of the cycle.
        int cycleLength = stack.Count - startIndex;
        // Allocate an array to hold the nodes forming the cycle.
        INode[] cycle = new INode[cycleLength];
        // Copy the nodes from the stack to the cycle array.
        for (int i = 0; i < cycleLength; i++)
        {
            cycle[i] = stack[startIndex + i];
        }
        return cycle;
    }
    ...
    private static INode[] NormalizeCycle(INode[] cycle)
    {
        ...
        INode[] normalizedCycle = new INode[cycle.Length];
        for (int i = 0; i < cycle.Length; i++)
        {
            normalizedCycle[i] = cycle[(minNodeIndex + i) % cycle.Length];
        }
