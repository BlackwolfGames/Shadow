...
public class Edge : IEdge
{
    public Edge(PreEdge edge, IDependencyGraph graph)
    {
        ...
        from = graph.Exact(edge.LhsName) ??
               throw new NullReferenceException($"There is no node named {edge.LhsName} in the dependency graph");
        to = graph.Exact(edge.RhsName) ??
             throw new NullReferenceException($"There is no node named {edge.RhsName} in the dependency graph");
    }
    ...
    public int SharedNamespaces => from.Namespaces().Zip(to.Namespaces())
        .TakeWhile((tuple, _) => tuple.First == tuple.Second).Count();
