...
public class ImplicitConversionAnalysis : DependencyStrategy<EqualsValueClauseSyntax>
{
  protected override IEnumerable<AnalysisResult> Analyze(EqualsValueClauseSyntax invocation, SemanticModel model)
  {
    ...
    ITypeSymbol? rhsType = ModelExtensions.GetTypeInfo(model, invocation.Value).Type; // Type of right-hand side
    ...
    ITypeSymbol? lhsType = ModelExtensions.GetTypeInfo(model, variableDeclaration.Type).Type; // Type of left-hand side

    if (lhsType?.BaseType?.Name == "MulticastDelegate")
    {
      return new[]
      {
        new AnalysisResult(true, DependencyType.SubscribesToDelegate, lhsType.ToDisplayString())
      };
    }
    ...
    return new[]
    {
      new AnalysisResult(true, DependencyType.ImplicitConversion, rhsType?.ToDisplayString() ?? "BROKEN <ImplicitConversion>")
    };
  }
}