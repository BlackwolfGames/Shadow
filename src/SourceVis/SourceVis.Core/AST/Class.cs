...
public class Class
{
  ...
  public void Print()
  {
    foreach (var dependency in _deps) Console.WriteLine($" -- -- Depends on:" + $" {dependency.Key} - {dependency.Value} times");
  }

  public void UpdateWith(IEnumerable<AnalysisResult> dependencyType)
  {
    foreach (AnalysisResult result in dependencyType.Where(result => result.Success)) AddDependency(result.Dependency, result.ClassName);
  }
}