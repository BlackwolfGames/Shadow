...
public class EdgeMetrics
{
  ...
  public void ThenTheAverageEdgeLengthIs(EqualClass eq, Expression p0)
  {
    ...
    Assert.That(edges.Select(edge => Vector2.Distance(edge.Start, edge.End)).Average(), eq.TestAs(p0.Evaluate()));
  }
  ...
  {
    var edges = _scenarioContext.Get<GraphProjection>().Edges.ToArray();
    Assert.That(edges.Select(edge => Vector2.Distance(edge.Start, edge.End)).Min(), eq.TestAs(p0.Evaluate()));
  }
  ...
  public void ThenTheMaximumEdgeLengthIs(EqualClass eq, Expression p0)
  {
    ...
    Assert.That(edges.Select(edge => Vector2.Distance(edge.Start, edge.End)).Max(), eq.TestAs(p0.Evaluate()));
  }
  ...
  {
    var edges = _scenarioContext.Get<GraphProjection>().Edges.ToArray();
    var distances = edges.Select(edge => Vector2.Distance(edge.Start, edge.End)).ToArray();
