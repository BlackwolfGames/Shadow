...
public class LayoutMetricSteps
{
  ...
  {
    Assert.That(_scenarioContext.Get<GraphProjection>()
        .Nodes.Sum(node => node.Position.Length()),
  ...
  public void ThenTheCentroidDistanceFromCenterIs(EqualClass eq, Expression p0)
  {
    ...
    var centerOfMass = new Vector2(sumX, sumY) / nodes.Length;
    Assert.That(centerOfMass.Length(),
      eq.TestAs(p0.Evaluate())
    );
  }
