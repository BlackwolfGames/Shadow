...
public sealed class NodeMetrics
{
  ...
  public void ThenTheAverageDistanceBetweenNodesIsD(EqualClass eq, Expression p0)
  {
    ...
    Assert.That(nodes.SelectMany((point, index) => nodes.Skip(index + 1).Select(otherPoint => Vector2.Distance(point, otherPoint))).Average(), eq.TestAs(p0.Evaluate()));
  }
  ...
  public void ThenTheMinimumDistanceBetweenNodesIsD(EqualClass eq, Expression p0)
  {
    ...
    Assert.That(nodes.SelectMany((point, index) => nodes.Skip(index + 1).Select(otherPoint => Vector2.Distance(point, otherPoint))).Min(), eq.TestAs(p0.Evaluate()));
  }
  ...
  public void ThenTheMaximumDistanceBetweenNodesIsD(EqualClass eq, Expression p0)
  {
    ...
    Assert.That(nodes.SelectMany((point, index) => nodes.Skip(index + 1).Select(otherPoint => Vector2.Distance(point, otherPoint))).Max(), eq.TestAs(p0.Evaluate()));
  }
  ...
  public void ThenTheStandardDeviationOfDistancesBetweenNodesIsD(EqualClass eq, Expression p0)
  {
    ...
    var distances = nodes.SelectMany((point, index) => nodes.Skip(index + 1).Select(otherPoint => Vector2.Distance(point, otherPoint))).ToArray();

    double mean = distances.Average();
    var sumOfSquaresOfDifferences = distances.Select(val => (val - mean) * (val - mean)).Sum();
    var standardDeviation = Math.Sqrt(sumOfSquaresOfDifferences / distances.Length);
