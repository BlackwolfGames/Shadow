...
public class ParsingSteps : LogHelper
{
  ...
  public ParsingSteps(ScenarioContext scenarioContext)
  {
  ...
  {
    LogAssert(() => Assert.That(_scenarioContext.Get<Project>().Classes.Select(pair => pair.Key.Split('.')[^1]), Contains.Item(className.Split('.')[^1])));
  }

  [Then(@"The class '(.*)' uses '(.*)' as (.*) (\d*) times?")]
  public void ThenTheClassDependsOnAsTimeS(
    string className,
    string dependencyName,
    DependencyType type,
    int dependencyCount)
  {
    LogAssert(() => Assert.That(_scenarioContext.Get<Project>().Classes.First(ContainsKey<Class>(className)).Value.Dependencies.First(ContainsKey<Dependency>(dependencyName)).Value[type], Is.EqualTo(dependencyCount)));
  }
  ...
  {
    LogAssert(() => Assert.That(_scenarioContext.Get<Project>().Classes.First(ContainsKey<Class>(className)).Value.Dependencies.First(ContainsKey<Dependency>(dependencyName)).Value.Total, Is.EqualTo(dependencyCount)));
  }
  ...
  {
    LogAssert(() => Assert.That(_scenarioContext.Get<Project>().Classes.First(ContainsKey<Class>(className)).Value.Dependencies.Count(), Is.EqualTo(dependencyCount)));
  }
